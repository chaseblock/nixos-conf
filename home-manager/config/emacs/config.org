#+TITLE: Chase's Emacs
#+AUTHOR: chaseblock
#+DESCRIPTION: Chase's Emacs, forked from Justinlime's Emacs
...
- [[#configurations][Configurations]]
  ...
  - [[#programming-modes][Programming Modes]]
    - [[#typst][Typst]]
    - [[#text-editing-mode][Text Editing Mode]]
...
*** Fonts
#+begin_src emacs-lisp
(setq cb/font-size (if cb/is-wsl 200 120))
(setq cb/mono-font "FantasqueSansM Nerd Font")
(set-frame-font "FantasqueSansM Nerd Font" nil t)
(defun cb/set-local-font (font-size)
...
(add-hook 'minibuffer-setup-hook (lambda ()
  ...
  (face-remap-add-relative 'fringe
                `(:background ,cb/bg-0 :foreground ,cb/bg-0))))

(add-hook 'post-command-hook (lambda ()
  (set-cursor-color (if (derived-mode-p 'dired-mode) cb/acc-1 cb/acc-0))))

(set-face-attribute 'line-number-current-line nil :foreground cb/acc-0)
(set-face-attribute 'org-block nil :background cb/bg-0) ; src blocks
(set-face-attribute 'org-block-begin-line nil :background cb/bg-0) ; #+begin_src
(set-face-attribute 'org-block-end-line nil :background cb/bg-0) ; #+end_src
(set-face-attribute 'mode-line nil :background cb/bg-0) ; mode line background
(set-face-attribute 'mode-line-inactive nil :background cb/bg-0) ; mode line background on inactive buffers
(set-face-attribute 'default nil :background cb/bg-1) ; emacs background
(set-face-attribute 'fringe nil :background cb/bg-1) ; fringes/borders on the sides
(set-face-attribute 'org-hide nil :foreground cb/bg-1) ; asterisks preceding org headings
(set-face-attribute 'line-number nil :background cb/bg-1) ; line number background
(set-face-attribute 'line-number-current-line nil :background cb/bg-1) ; current line number
(set-face-attribute 'vertical-border nil :background cb/bg-0 :foreground cb/bg-0)
#+end_src
*** Misc
#+begin_src emacs-lisp
;; emacs -nw settings
...
(use-package eat
  ...
  :hook (eat-mode . (lambda ()
    ...
    (face-remap-set-base 'default :background cb/bg-0)
    (face-remap-set-base 'fringe :background cb/bg-0)))
  :config
...
(leader
  "s u" '(cb/ssh-user :wk "SSH as user, using the ssh config file")
  "s r" '(cb/ssh-root :wk "SSH as user with root privledges, using the ssh config file")
  "s m" '(cb/samba :wk "Access an SMB share"))
...
(defun cb/dired-open ()
  ...
  (let ((file (dired-get-file-for-visit)))
    (if (member (file-name-extension file) '("mp4" "mkv" "mov" "flac" "webm" "mp3" "ogg" "opus" "aac"))
      ...
      (dired-find-file))))
(defun cb/clear-marks-advice (&rest r)
...
(add-hook 'dired-mode-hook (lambda () 
  ...
  (face-remap-add-relative 'hl-line
                `(:foreground ,cb/bg-0 :background ,cb/acc-1 :extend t))
  (setq-local cursor-type nil
              evil-force-cursor t
              auto-revert-verbose nil)
  (cb/set-local-font (+ cb/font-size 10))
  (dired-omit-mode)
  (auto-revert-mode)
...
(advice-add #'dired-do-delete :after #'cb/clear-marks-advice)
(advice-add #'dired-do-rename :after #'cb/clear-marks-advice)
(advice-add #'dired-do-flagged-delete :after #'cb/clear-marks-advice)
(general-define-key
  :states 'normal
...
(use-package diredfl 
  ...
  :config
    (set-face-attribute 'diredfl-dir-heading nil :height (+ cb/font-size 20) :foreground cb/acc-0))
#+end_src
*** Dired Filter
#+begin_src emacs-lisp
...
(add-hook 'term-mode-hook (lambda ()
  ...
  `((t (:inherit default :background ,cb/bg-0)))
...
(add-hook 'org-mode-hook #'cb/org-resize-inline)
(add-hook 'after-save-hook #'cb/org-resize-inline)
;; Modified from org-rog to work with grim and slurp
(defun cb/org-grim-slurp ()
...
*** Org Last Modified
#+begin_src emacs-lisp 
(defun cb/org-find-time-file-property (property &optional anywhere)
...
(defun cb/org-has-time-file-property-p (property &optional anywhere)
  "Return the position of time file PROPERTY if it is defined.

As a special case, return -1 if the time file PROPERTY exists but
is not defined."
  (when-let ((pos (cb/org-find-time-file-property property anywhere)))
    (save-excursion
      (goto-char pos)
      (if (and (looking-at-p " ")
               (progn (forward-char)
...
(defun cb/org-set-time-file-property (property &optional anywhere pos)
...
it can be passed in POS."
  (when-let ((pos (or pos
                      (cb/org-find-time-file-property property))))
    (save-excursion
      (goto-char pos)
      (if (looking-at-p " ")
          (forward-char)
...
(defun cb/org-set-last-modified ()
  "Update the LAST_MODIFIED file property in the preamble."
  (when (derived-mode-p 'org-mode)
    (cb/org-set-time-file-property "modified")))
(add-hook 'before-save-hook #'cb/org-set-last-modified)
#+end_src
...
(let ((dir "~/sync/notes/roam"))
  (if (file-directory-p dir)
      (setq cb/org-roam-dir dir)
      (setq cb/org-roam-dir (expand-file-name "org/roam" user-emacs-directory))
      (make-directory cb/org-roam-dir t)))
  ...
  :config
    (cl-defmethod org-roam-node-type ((node org-roam-node))
      ...
      (condition-case nil
        ...
        (error "")))
    (setq cb/roam-templ "#+created: %U\n#+modified:\n#+export_select_tags: noexport\n#+options: f:nil toc:nil\n#+title: ${title}\n#+filetags:"
          org-roam-node-display-template (concat 
                                           (propertize "${type:10}" 'face 'org-level-2) 
                                           "${title:100}" 
                                           (propertize "${tags:*}" 'face 'org-level-1))
          org-roam-directory (file-truename cb/org-roam-dir)
          org-roam-capture-templates  
            `(("w" "work" plain "%?" :target
                (file+head "work/%<%Y%m%d%H%M%S>-${slug}.org" ,cb/roam-templ)
                  :immediate-finish t
                  :unnarrowed t)
              ("g" "general" plain "%?" :target
                (file+head "general/%<%Y%m%d%H%M%S>-${slug}.org"  ,cb/roam-templ)
                  :immediate-finish t
                  :unnarrowed t)
              ("s" "scratch" plain "%?" :target
                (file+head "scratch/%<%Y%m%d%H%M%S>-${slug}.org" ,cb/roam-templ)
                  :immediate-finish t
                  :unnarrowed t)))
...
(use-package olivetti
  ...
  :config
    (set-face-attribute 'olivetti-fringe nil :background cb/bg-1) ; fringes/borders on the sides
...
*** Consult
#+begin_src emacs-lisp
(defun cb/find-in-dir (&optional directory)
...
(defun cb/grep-in-dir (&optional directory)
...
(defun cb/switch-to-buffer ()
  "Use consult with perspective buffers, and also filter out dired buffers"
  (interactive)
  (defvar cb/filter-orig-func (plist-get persp-consult-source :items))
  (plist-put persp-consult-source :items (lambda () 
    (let ((candidates (funcall cb/filter-orig-func)))
...
(use-package consult
  ...
  :general
    (leader
      "b f" '(cb/switch-to-buffer :wk "Find a buffer, or create a new one")
      "b d" '(cb/switch-to-dired :wk "Find a dired buffer, or create a new one")
      "f b" '(consult-bookmark :wk "Find a bookmark")
      "f r" '(cb/find-in-current :wk "Find file in current dir/project")
      "f R" '(cb/find-in-dir :wk "Find file in specified dir/project")
      "f h" '((lambda () (interactive) (cb/find-in-dir "~")) :wk "Find file in $HOME")
      "f H" '((lambda () (interactive) (cb/find-in-dir "/")) :wk "Find file in /")
      "f w" '(cb/grep-in-current :wk "Find word in current dir/project")
      "f W" '(cb/grep-in-dir :wk "Find word in specified dir/project"))
  :config
...
(use-package corfu
  ...
  :config
    ...
    ;; Rice it up 
    (set-face-attribute 'corfu-default nil :background cb/bg-0)
    (set-face-attribute 'corfu-current nil :box cb/acc-0)
    (set-face-attribute 'corfu-border nil :background cb/acc-1)
    (set-face-attribute 'corfu-bar nil :background cb/acc-0)
...
*** Dashboard
#+begin_src emacs-lisp
(defun cb/random-quote ()
  "Generate a random quote for dashboard"
  (interactive)
  (shell-command-to-string "fortune"))

(use-package dashboard
  :hook (dashboard-mode . (lambda () (cb/set-local-font (+ cb/font-size 10))))
  :config
    (setq default-directory "~/"
          ...
          dashboard-banner-logo-title (cb/random-quote)
          dashboard-footer-messages `(,(cb/random-quote)))
...
(use-package sideline
  ...
  :config
    (set-face-attribute 'sideline-default nil :foreground cb/acc-0)
...
*** Typst
#+begin_src emacs-lisp
(defun my/typst-fill-paragraph (&optional justify)
  "Fill the current Typst list item without merging adjacent items."
  (let* ((node (treesit-node-at (point)))
         (item (catch 'found
                 (while node
                   (when (string= (treesit-node-type node) "item")
                     (throw 'found node))
                   (setq node (treesit-node-parent node))))))
    (if item
        (save-restriction
          ;; Restrict filling to this list item's contents.
          (narrow-to-region
           (treesit-node-start item)
           (treesit-node-end item))
          ;; Skip the list marker (`-`, `+`, etc.).
          (goto-char (point-min))
          (when (looking-at "[ \t]*[-+*][ \t]+")
            (goto-char (match-end 0)))
          (let ((fill-paragraph-function nil))
            (fill-paragraph justify)))
      ;; Not in a list: ordinary M-q behavior.
      (let ((fill-paragraph-function nil))
        (fill-paragraph justify)))))
...
(use-package doom-modeline
  :config
    (set-face-attribute 'doom-modeline nil :background cb/bg-0) ; doom mode line background
...
(use-package nerd-icons
  :config
    (setq nerd-icons-font-family cb/mono-font))
#+end_src
**** Nerd Icons Dired
#+begin_src emacs-lisp
...
(use-package pulsar
  ...
  :config 
    (set-face-attribute 'pulsar-generic nil :background cb/acc-0)
